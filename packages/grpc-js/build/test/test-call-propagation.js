...
describe('Call propagation', () => {
    ...
    let proxyClient;
    before((done) => {
        Client = common_1.loadProtoFile(__dirname + '/fixtures/test_service.proto').TestService;
    ...
    describe('Deadlines', () => {
        it('should work with unary requests', (done) => {
            ...
            call = proxyClient.unary({}, { deadline }, (error, value) => {
                assert(error);
                assert.strictEqual(error.code, grpc.status.DEADLINE_EXCEEDED);
                done();
            });
        ...
        it('Should work with client streaming requests', (done) => {
            ...
            const deadline = new Date();
            deadline.setMilliseconds(deadline.getMilliseconds() + 100);
            call = proxyClient.clientStream({ deadline, propagate_flags: grpc.propagate.DEADLINE }, (error, value) => {
                assert(error);
                assert.strictEqual(error.code, grpc.status.DEADLINE_EXCEEDED);
                done();
            });
